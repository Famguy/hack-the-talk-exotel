...
But unlike fread, returns -\/1 if required number of elements (n\-\_\-el) not read; also, no byteswapping or checksum accumulation is performed in that case. 
...
\item[{uint32 $\ast$}]{chksum}
\end{DoxyParamCaption}
)}\label{bio_8h_a3b131beb20492e1a6f46c9b4dd81a64e}
...
\begin{DoxyParams}{Parameters}
{\em directory} & the folder where the file is located \\
\hline
{\em filename} & the name of the file \\
\hline
{\em extension} & file extension \\
\hline
{\em header} & the size of the header to skip usually 44 bytes. \\
\hline
{\em endian} & Endian of the data \\
\hline
{\em nsamps} & number of samples read \\
\hline
\end{DoxyParams}
\begin{DoxyReturn}{Returns}
pointer to the data 
